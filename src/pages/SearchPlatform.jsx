...
const SearchPlatform = ({ setUrl }) => {
  const { fetchPlatformLength, fetchGamesPlatform } = useAuth();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchPlatformLength(platform);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await fetchGamesPlatform(
          platform,
          currentPage,
          itemsPerPage
        );
    ...
    fetchData();
  }, [platform, currentPage, itemsPerPage]);
        ...
        {game && !loading && (
          <div className="mt-5 ">
            <p className="text-3xl">
              Searching Games in <span className="underline">{platform}</span>
            </p>
            {game &&
              game.map((e, i) => (
                <div
