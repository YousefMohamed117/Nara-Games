...
const SearchGenres = ({ setUrl }) => {
  const { fetchGenresLength, fetchGenres, fetchThemes, fetchGamesByFilters } =
    useAuth();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchGenresLength(selectedGenres, selectedThemes);
    ...
    fetchData(selectedGenres, selectedThemes);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await fetchGamesByFilters(
          selectedGenres,
          selectedThemes,
          currentPage,
          itemsPerPage
    ...
    fetchData();
  }, [selectedGenres, selectedThemes, currentPage]);
        ...
        {themes && genres && (
          <>
            <div className="filters mt-4">
              <div
                className={`filter-list mt-4 ${showGenres ? "h-auto" : "h-4"}`}
              >
                <h2>
                  Genres{" "}
                  <FontAwesomeIcon
                    icon={showGenres ? faCaretUp : faCaretDown}
                    onClick={() => setShowGenres(!showGenres)}
                  />
                </h2>
                <div
                  className={`themes-list mt-4 duration-300 flex flex-wrap gap-2  ${
                    showGenres ? "-translate-y-[0]" : "-translate-y-[300px]"
                  } `}
                >
                  {genres.map((genre) => (
                    <span
              ...
              <div
                className={`filter-list mt-4 ${showThemes ? "h-auto" : "h-4"}`}
              >
                <h2>
                  Themes{" "}
                  <FontAwesomeIcon
                    icon={showThemes ? faCaretUp : faCaretDown}
                    onClick={() => setShowThemes(!showThemes)}
                    className="cursor-pointer"
                  />
                </h2>
                <div
                  className={`themes-list mt-4 duration-300 flex gap-2  flex-wrap ${
                    showThemes ? "-translate-y-[0]" : "-translate-y-[300px]"
                  } `}
                >
                  {themes.map((theme) => (
                    <span
