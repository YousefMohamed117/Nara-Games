...
const SearchCompany = ({ setUrl }) => {
  const { fetchCompanyLength, fetchGamesCompany } = useAuth();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchCompanyLength(company);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await fetchGamesCompany(
          company,
          currentPage,
          itemsPerPage
        );
        ...
        {game && !loading && (
          <div className="mt-5 ">
            <p className="text-3xl">
              Searching Games in <span className="underline">{company}</span>
            </p>
            {game &&
              game.map((e, i) => (
                <div
