...
const Search = ({ setUrl }) => {
  const { fetchSearch, fetchLength } = useAuth();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        if (search) {
          const data = await fetchLength(search);
          if (data && data.length > 0) {
            setTotalPages(Math.round(data.length / 10));
          }
        } else {
          setTotalPages(50); // Set a default total pages value when no search query is provided
        }
      } catch (error) {
        Navigate("/error");
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const data = await fetchSearch(search, currentPage, itemsPerPage);
        if (data && data.length > 0) {
      ...
      {!loading && (
        <>
          ...
          <div className="mt-[76px] ">
            {search && !error && (
              <h1 className="text-3xl mb-4 w-full">
                Your Search Results for {search}
              </h1>
            )}
            {error && (
              <p className="absolute top-[300px] left-1/2 -translate-x-1/2 text-3xl text-red-600">
                {error}
              </p>
            )}
            {game &&
