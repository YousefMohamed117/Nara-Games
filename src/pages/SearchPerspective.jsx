...
const SearchPerspective = ({ setUrl }) => {
  const { fetchPerspectiveLength, fetchGamePerspective } = useAuth();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchPerspectiveLength(perspective);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await fetchGamePerspective(
          perspective,
          currentPage,
          itemsPerPage
        );
    ...
    fetchData();
  }, [perspective, currentPage, itemsPerPage]);
        ...
        {game && !loading && (
          <div className="mt-5 ">
            <p className="text-3xl">
              Searching Games in{" "}
              <span className="underline">{perspective}</span>
            </p>
            {game &&
              game.map((e, i) => (
                <div
...
export default SearchPerspective;
