...
const SearchGenres = ({ setUrl }) => {
  const { fetchGenreLength, fetchGamesGenre } = useAuth();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchGenreLength(genre);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await fetchGamesGenre(genre, currentPage, itemsPerPage);
        if (data && data.length > 0) {
    ...
    fetchData();
  }, [genre, currentPage, itemsPerPage]);
        ...
        {game && !loading && (
          <div className="mt-5 ">
            <p className="text-3xl">
              Searching Games in <span className="underline">{genre}</span>
            </p>
            {game &&
              game.map((e, i) => (
                <div
                ...
                >
                  <Link to={`/game/${e.id}`}>
                    <img
                      src={e.cover.url}
                      alt=""
                      className="sm:h-full h-[300px] sm:w-[264px] min-w-[264px]  w-full p-2 rounded-2xl  object-cover "
                    />
                  </Link>
